    ...
    if love.keyboard.isDown('lshift') then
        if player.grounded then --if there is a collider below the player
            ...
            player:applyLinearImpulse(player.direction * 3000 , 0) -- Apply kick force in the direction player is facing
            boost_sound:play()
        else
            player.iskicking = false
            player.iswalking = false
    ...
    if player.grounded then
        if vy < 20 and vy > -20 then
            ...
            elseif player.isteleporting or player.isshooting then
                player.animation = animations.teleport
            elseif isSlashing then
                player.animation = animations.slash
            elseif player.isboosting then
                player.animation = animations.boost
      
            elseif player.mattacking2 then
                player.animation = animations.slash2
            -- elseif player.mattacking2 then
            --     player.animation = animations.slash2
            -- elseif player.isslashing2 then
            --     player.animation = animations.slash2
            -- elseif player.isslashing2 then
            --     player.animation = animations.slash2
            -- elseif player.isslashing2 then
    ...
    love.graphics.printf(slashTimer,px,py - 100,60)
        

    love.graphics.printf(tostring(isSlashing),px,py - 200,60)
